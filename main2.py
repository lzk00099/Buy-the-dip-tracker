...
def calculate_quant_and_breadth_signals():
    """大盘 ETF 与 CBOE 官方数据混合对齐引擎（引入微观动态诊断逻辑）"""
    try:
...
switches = [
    ...
    },
    {
        ...
        "fetched_status": "数据抓取失败 🔴" if crypto_data["error"] else (
            "🚨 预警：多头杠杆超载过热" if crypto_data["top_active"] else (
                "🟢 激活：空头无脑割肉出清" if crypto_data["bottom_active"] else "⚪ 中性：离岸高杠杆状态稳定"
            )
        ),
...
if top_score >= 3 and top_score >= bottom_score:
    current_global_state = "TOP_ALERT"
    status_color = "red"
    action_title = "🚨 【红色防御：触发系统性风控见顶逃顶线】"
    action_text = (
        f"**大盘底层资金面诊断**：当前系统5大核心开关中有 **{top_score}** 项联合拉响见顶警报，市场呈现极度贪婪或高位严重分裂！<br>"
        "做市商Gamma敞口恶化引发追跌放大效应，配合暗池主力高位派发及多头边际买盘枯竭。此时必须**全面收紧个股诊断模型的止盈线**，"
        "转入全面战略防御，严控多头杠杆。"
    )
elif bottom_score >= 3 and bottom_score >= top_score:
    current_global_state = "BOTTOM_ACTIVE"
    status_color = "green"
    action_title = "🚀 【绿色共振：底部恐慌宣泄枯竭，触发拐点重仓抄底】"
    action_text = (
        f"**大盘底层资金面诊断**：当前系统5大核心开关中有 **{bottom_score}** 项指标达成共振，黄金左侧买点确立！<br>"
        "做市商Gamma转正提供防护，暗池机构大单托底，且离岸高杠杆和系统性CTA抛压均已砸至历史冰点。多头精准抄底模式全面启动，"
        "建议结合个股的 Expected Value 与 Random Forest 模型全力捕捉大盘错杀的EV红利股。"
    )
else:
    current_global_state = "NEUTRAL"
    status_color = "orange"
    action_title = "⏳ 【黄色震荡：多空状态均衡交织，常规结构分化轮动】"
    action_text = (
        f"**大盘底层资金面诊断**：当前系统无极端单边共振信号（抄底激活:{bottom_score} | 见顶风控:{top_score} | 状态中性:{neutral_score}）。"
...
with c1:
    st.metric(label="🚀 底部抄底激活数", value=f"{bottom_score} / 5", delta="多头共振" if bottom_score >= 3 else None)
with c2:
    st.metric(label="🚨 见顶风控激活数", value=f"{top_score} / 5", delta="-高位防守" if top_score >= 3 else None, delta_color="inverse")
with c3:
    st.metric(label="⏳ 状态中性均衡数", value=f"{neutral_score} / 5")
...
if not ndx_data.empty:
    ...
    fig_ndx.update_layout(
        title="Nasdaq 100 (^NDX) 阶梯支撑与洗盘推推演 (智能自适应缩放)",
